...
var index = require('./index-26d8486e.js');
require('events');
require('stream');
...
function parent(str, { strict = false } = {}) {
  str = path$1.normalize(str).replace(/\/|\\/, '/');
...
function walk(output, prefix, lexer, opts, dirname='', level=0) {
  ...
  for (; i < len; i++) {
    ...
    if ((stats=CACHE[relpath]) === void 0) {
      CACHE[relpath] = stats = fs$1.lstatSync(fullpath);
    }
...
var symbols = index.createCommonjsModule(function (module) {
...
var utils = index.createCommonjsModule(function (module, exports) {
...
var ansi_1 = index.createCommonjsModule(function (module, exports) {
...
class Prompt extends index.Events {
  constructor(options = {}) {
    super();
...
var prompts = index.createCommonjsModule(function (module, exports) {
...
class Enquirer extends index.Events {
  ...
  register(type, fn) {
    ...
    index.assert.equal(typeof fn, 'function', 'expected a function');
  ...
  async ask(question) {
    ...
    index.assert(this.prompts[type], `Prompt "${type}" is not registered`);
  ...
  static get prompt() {
    ...
    utils.mixinEmitter(fn, new index.Events());
    return fn;
  }
}

utils.mixinEmitter(Enquirer, new index.Events());
...
const args = lib(process.argv.slice(2), {
	alias: {
		f: 'force',
		c: 'cache',
		v: 'verbose',
...
const [src$1, dest = '.'] = args._;

async function main() {
	if (args.help) {
		const help = fs$1
			.readFileSync(path$1.join(__dirname, 'help.md'), 'utf-8')
			.replace(/^(\s*)#+ (.+)/gm, (m, s, _) => s + index.chalk.bold(_))
			.replace(/_([^_]+)_/g, (m, _) => index.chalk.underline(_))
			.replace(/`([^`]+)`/g, (m, _) => index.chalk.cyan(_));
		...
		sync(`**/access.json`, { cwd: index.base }).forEach(file => {
			const [host, user, repo] = file.split(path$1.sep);

			const json = fs$1.readFileSync(`${index.base}/${file}`, 'utf-8');
		...
		const getChoice = file => {
			...
			return Object.entries(index.tryRequire(`${index.base}/${file}`)).map(
				([ref, hash]) => ({
					name: hash,
		...
		const choices = sync(`**/map.json`, { cwd: index.base })
			.map(getChoice)
			.flat()
			.sort((a, b) => {
		...
		const options = await enquirer.prompt([
			{
				...
				suggest: (input, choices) =>
					choices.filter(({ value }) => fuzzysearch_1(input, value)),
				choices
			},
			{
				type: 'input',
		...
		const empty =
			!fs$1.existsSync(options.dest) || fs$1.readdirSync(options.dest).length === 0;
		...
		if (!empty) {
			...
			if (!force) {
				console.error(index.chalk.magenta(`! Directory not empty — aborting`));
				return;
			}
		}
...
function run(src, dest, args) {
	const d = index.degit(src, args);

	d.on('info', event => {
		console.error(index.chalk.cyan(`> ${event.message.replace('options.', '--')}`));
	});

	d.on('warn', event => {
		console.error(
			index.chalk.magenta(`! ${event.message.replace('options.', '--')}`)
		);
	});

	d.clone(dest).catch(err => {
		console.error(index.chalk.red(`! ${err.message.replace('options.', '--')}`));
		process.exit(1);
	});
}
